...
fn main() {
    ...
    println!("Please enter a temperature followed by either a F or C denoting celius or farenheit respectively: ");
    loop {
        ...
        if temp_type.eq_ignore_ascii_case("f") {
            let temp_in_celcius = convert_from_farenheit_to_celcius(temp);
            println!("{temp}°F converted to celcius is: {temp_in_celcius:.2}°C \n");
        } else if temp_type.eq_ignore_ascii_case("c") {
            let temp_in_farenheit = convert_from_celcius_to_farenheit(temp);
...
fn convert_from_farenheit_to_celcius(temp: f64) -> f64 {
    let celcius: f64 = 5.0 / 9.0 * (temp as f64 - 32f64);
    celcius
}

fn convert_from_celcius_to_farenheit(temp: f64) -> f64 {
